...
- T-COMPLETE: A transaction is complete when (a) the first event of a new cdcxid is observed, OR (b) a HEARTBEAT is received after all events of the transaction have been delivered. Condition (b) relies on `session_aware: true` -- the producer detects idle periods after transactions and emits a HEARTBEAT within the configured heartbeat_interval_seconds (T-HEARTBEAT). When `session_aware` is false, producers SHOULD still emit HEARTBEAT events at the configured interval; consumers MAY use condition (b) for transaction completion detection if the HEARTBEAT interval and stream ordering guarantees are sufficient for their use case. Consumers MUST NOT commit a transaction until T-COMPLETE is satisfied by one of the two conditions above.
...
HEARTBEAT events are interleaved in the data stream alongside DML and DDL events. When `session_aware` is true, they serve a dual role: liveness signaling and transaction completion signaling. A HEARTBEAT received after all events of a transaction confirms that the transaction is complete (T-COMPLETE condition (b), Section 8.3). Session-aware producers MUST emit a HEARTBEAT within heartbeat_interval_seconds after any transaction followed by an idle period (T-HEARTBEAT). When `session_aware` is false, producers SHOULD still emit periodic HEARTBEAT events for liveness signaling; consumers MAY also use them for transaction completion inference since single channel ordering (T-NOINTERLEAVE) is still guaranteed.
